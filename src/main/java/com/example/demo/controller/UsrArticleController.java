...
public class UsrArticleController {
	...
	public String getArticleAction(int id, Model model) {
		Article article = articleService.getArticle(id);

		if (article == null) {
			 model.addAttribute("resultData", ResultData.from("F-1", Ut.f("%d번 게시물은 존재하지 않습니다", id)));
		}else {
			model.addAttribute("resultData", ResultData.from("S-1", Ut.f("%d번 게시물입니다.", id), "article", article));
		}
